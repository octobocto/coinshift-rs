...
mod test {
    use crate::{
        state::{Error, tests::test_state},
        types::{Address, Output, OutputContent},
    };
    ...
    fn validation_rejects_outpoint_utxo_hash_mismatch() -> anyhow::Result<()> {
        ...
        // The header the validator expects: merkle_root from the filled tx,
        // roots = post-block accumulator built from the SUPPLIED utxo_hash.
        let filled = {
        ...
        anyhow::ensure!(matches!(
            state.validate_filled_transaction(&filled),
            Err(Error::UtxoHashMismatch { .. })
        ));
        let merkle_root =
...

        {
            ...
            anyhow::ensure!(
                ...
                and utxo_hash (B) refer to different UTXOs: {result:?}",
            );
        }
        Ok(())
    }
